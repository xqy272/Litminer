...
def _run_namespace(args: dict):
    ...
    return _argparse.Namespace(
        ...
        merge_into=_optional_workspace_path(
            args.get("merge_into"),
            "merge_into",
            must_exist=True,
        ),
...
TOOLS: dict[str, dict] = {
    ...
    "litminer_run_lit_search": {
        ...
        "parameters": {
            ...
            "merge_into": {"type": "string", "required": False, "description": "Existing Litminer output directory to merge a new iteration into; writes delta_profile and research_session_manifest"},
            ...
            "stop_after_stage": {"type": "string", "required": False, "description": "Stop after query_plan, discovery, merge, dedupe, pretriage, verification_queue, crossref, triage, selection, unpaywall, metrics, queue, or probe"},
            ...
            "max_crossref_rows": {"type": "integer", "required": False, "description": "Budget for unresolved rows from the priority verification queue; reusable prior results do not consume it"},
