...
Codex 支持在 `config.toml` 中注册包含 `SKILL.md` 的 skill 目录。可使用用户级 `~/.codex/config.toml`，也可以在可信项目中使用项目级 `.codex/config.toml`。`path` 必须指向你实际克隆出的 Litminer 目录。
...
```toml
[[skills.config]]
path = "/home/your-name/.claude/skills/litminer"
enabled = true
```
